  ...
  useEffect(() => {
    const fetchNearbyUsers = async () => {
      try {
        ...
        const users =
          data?.nearbyUsers?.map((user: any) => user.username) ?? [];
    ...
    <div className="container mx-auto py-8">
                  ...
                  users.map((name, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          onClick={() => trackUserLocation(name)}
                          disabled={loading.location}
                        >
                          {loading.location && selectedUser?.username === name ? (
      ...
      {mapVisible && selectedUser && (
        <Card className="mt-6">
          <CardHeader>
            <CardTitle>
              Tracking: <span className="text-primary">{selectedUser.username}</span>
            </CardTitle>
          ...
          <CardFooter className="text-sm text-muted-foreground">
            Coordinates: {selectedUser.latitude.toFixed(6)}, {selectedUser.longitude.toFixed(6)}
          </CardFooter>
        </Card>
      )}
    </div>
